...
export function sumRangeOverlaps(tuplePairs: Tuple[][]): number {
...
export function sumFullyOverlappedSectionAssignments(
  assignments: string[]
): number {
  ...
  return sumRangeOverlaps(tuplePairs);
}
...
export function run() {
  readInputFileToStringArray(
    ...
    (err, data) => {
      if (!err) {
        console.log(
          `The sum of fully overlapped assignment pairs: ${sumFullyOverlappedSectionAssignments(
            data
          )}`
        );
      } else {
